---
name: longneck
description: "既存のLongneckタスク（.longneck/TITLE/）を進める親オーケストレーター。実行可能なタスクを1つずつ新しい子エージェントへ委譲し、成果を実ファイルで照合しながら、完了するか、残件が判断待ち・ブロック中（とその解消を待つ依存タスク）だけになるか、進捗が得られなくなるまで進める。ユーザーが $longneck を指定した場合や、Longneckタスクの継続・再開・残件消化を依頼した場合に使う。Longneck形式でない STATE / PROGRESS には使わず、状態確認、新規作成、定義改訂、形式修復、永続化、スリム化、削除は各専用スキルに委ねる。"
---
