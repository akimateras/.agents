...
# Separators match task-format.md exactly: whitespace, 読点, or comma.
...
# retired IDs from being reused. T and I also scan each other's home document
# so a cross-referenced ID (a T-ID cited in issues.md, an I-ID cited in
# state.md) is never re-issued even when the work log missed it.
ID_SOURCE_FILES = {
    "T": ("state.md", "progress.md", "issues.md"),
    "D": ("progress.md",),
    "I": ("state.md", "progress.md", "issues.md"),
}
...
SNAPSHOT_SECTION = "現在のスナップショット"
STATE_SECTIONS = ("調査待ち", "判断待ち", "実行待ち", "ブロック中")
DESCRIPTION_SECTIONS = ("目的", "スコープ", "非スコープ", "完了条件", "長期制約", "主な作業対象")
...
def _git_stdout(directory: Path, *arguments: str) -> str | None:
    """Return git stdout, or None when git is unavailable or the call fails."""
    try:
...
def resolve_root(start: Path) -> Path:
    ...
    bare = _git_stdout(start_path, "rev-parse", "--is-bare-repository")
    if bare is not None and bare.strip() == "true":
        raise LongneckError("bare repositories do not have a Longneck working root")
    toplevel = _git_stdout(start_path, "rev-parse", "--show-toplevel")
    if toplevel is None:
        return start_path

    output = toplevel.strip()
    if not output:
...
def _unique_ids(
    ...
    named_heading = re.compile(rf"^### ({prefix}-[0-9]{{4,}}) — .+$")
    ids: list[str] = []
    for line in text.splitlines():
        match = pattern.match(line)
        if match:
            ids.append(match.group(1))
            if not named_heading.match(line):
                warnings.append(
                    f"{label} {match.group(1)} has no name in its heading"
                )
...
def _parse_state(
    ...
    for line in text.splitlines():
        ...
        if not TASK_HEADING_NAME_PATTERN.match(line):
            # Counted here but invisible to _extract_tasks: without this error
            # the task would never appear in the tasks report and never run.
            errors.append(
                f"malformed task heading (expected '### {task_id} — <名前>'): "
                f"{line.strip()}"
            )
    ...
    if SNAPSHOT_SECTION in all_sections and all_sections[0] != SNAPSHOT_SECTION:
        errors.append(
            f"state.md must place '## {SNAPSHOT_SECTION}' as the first section"
        )
    for heading in all_sections:
        if heading != SNAPSHOT_SECTION and heading not in STATE_SECTIONS:
            # Extra sections are where completed-feature lists and design notes
            # accumulate; flag them as bloat for longneck-flush to propose.
            warnings.append(f"non-standard state.md section heading: ## {heading}")
...
def _catalog_state(
    state: str, title: str, errors: list[str], warnings: list[str]
) -> tuple[str | None, dict[str, int], str | None]:
    ...
    return _parse_state(state, errors, warnings)
...
def validate_task_path(path: Path, title: str) -> ValidationResult:
    ...
    if description is not None:
        ...
        for section in DESCRIPTION_SECTIONS:
            _require_heading(description, f"## {section}", "description.md", errors)
        seen_sections = [
            line[3:]
            for line in description.splitlines()
            if line.startswith("## ") and line[3:] in DESCRIPTION_SECTIONS
        ]
        if seen_sections != list(DESCRIPTION_SECTIONS) and all(
            seen_sections.count(section) == 1 for section in DESCRIPTION_SECTIONS
        ):
            errors.append(
                "description.md sections are out of order; expected "
    ...
    if state is not None:
        _require_heading(state, f"## {SNAPSHOT_SECTION}", "state.md", errors)
        lifecycle, counts, updated = _catalog_state(state, title, errors, warnings)
    ...
    if progress is not None:
        ...
        _unique_ids(progress, DECISION_ID_PATTERN, "D", "decision", errors, warnings)
    ...
    if issues is not None:
        ...
        _unique_ids(issues, ISSUE_ID_PATTERN, "I", "issue", errors, warnings)
...
def tasks_report(root: Path, title: str) -> dict[str, Any]:
    ...
    lifecycle, counts, updated = _parse_state(state, errors, warnings)
...
def _list_locks(base: Path) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    if locks_dir.is_symlink():
        ...
        return locks, errors
    if not os.path.lexists(locks_dir):
        return locks, errors
    if not locks_dir.is_dir():
        errors.append(f"locks directory is not a directory: {locks_dir}")
        return locks, errors
