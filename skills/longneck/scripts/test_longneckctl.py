...
class LongneckCtlTest(unittest.TestCase):
    ...
    def missing_git_path(self) -> unittest.mock._patch_dict:
        # An empty PATH makes every "git" invocation raise FileNotFoundError,
        # simulating an environment without a git binary.
        return unittest.mock.patch.dict(
            os.environ, {"PATH": str(self.root / "no-binaries")}
        )

    def test_resolve_root_without_git_binary_falls_back_to_start(self) -> None:
        nested = self.root / "nested"
        nested.mkdir()
        with self.missing_git_path():
            self.assertEqual(longneckctl.resolve_root(nested), nested.resolve())

    def test_backlinks_without_git_binary_use_walk_mode(self) -> None:
        self.create()
    ...
    def test_snapshot_must_be_the_first_section(self) -> None:
        self.create()
        state = self.task_path("state.md")
        text = state.read_text(encoding="utf-8")
        snapshot = "## 現在のスナップショット\n\n初期状態。\n\n"
        moved = text.replace(snapshot, "").replace(
            "## 判断待ち", snapshot + "## 判断待ち"
        )
        self.assertNotEqual(text, moved)
        state.write_text(moved, encoding="utf-8")
        result = longneckctl.validate_task(self.root, "sample-task")
        self.assertFalse(result.ok)
        self.assertTrue(any("first section" in error for error in result.errors))

    def test_non_standard_state_section_is_a_warning(self) -> None:
        self.create()
        state = self.task_path("state.md")
        text = state.read_text(encoding="utf-8")
        ...
        self.assertTrue(
            any(
                "non-standard state.md section" in warning
    ...
    def test_task_heading_with_bad_name_separator_is_an_error(self) -> None:
        # A heading like "### T-0001 調査する" is counted by the state parser
        # but silently dropped from the tasks report; it must be an error so
        # the task cannot vanish from scheduling without a diagnostic.
        for heading in ("### T-0001 調査する", "### T-0001 —", "### T-0001—調査する"):
            with self.subTest(heading=heading):
                shutil.rmtree(self.root / ".longneck", ignore_errors=True)
                self.create()
                state = self.task_path("state.md")
                text = state.read_text(encoding="utf-8")
                state.write_text(
                    text.replace("### T-0001 — 調査する", heading), encoding="utf-8"
                )
                result = longneckctl.validate_task(self.root, "sample-task")
                self.assertFalse(result.ok)
                self.assertTrue(
                    any("malformed task heading" in error for error in result.errors),
                    result.errors,
                )
                report = longneckctl.tasks_report(self.root, "sample-task")
                self.assertTrue(
                    any("malformed task heading" in error for error in report["errors"])
                )
    ...
    def test_sentence_final_period_makes_dependency_free_text(self) -> None:
        # task-format.md allows only whitespace, 読点, and comma separators;
        # a 句点 turns the line into free text left to the caller's judgement.
        self.create()
        self.append_pending_task("T-0002", "T-0001。")
        report = longneckctl.tasks_report(self.root, "sample-task")
        by_id = {task["id"]: task for task in report["tasks"]}
    ...
    def test_list_reports_locks_path_that_is_not_a_directory(self) -> None:
        self.create()
        (self.root / ".longneck" / ".locks").write_text("{}\n", encoding="utf-8")
        listing = longneckctl.list_tasks(self.root)
        self.assertTrue(
            any("is not a directory" in error for error in listing["entry_errors"])
        )
    ...
    def test_next_task_id_scans_state_progress_issues_and_archive(self) -> None:
        self.create()
        result = longneckctl.next_id(self.root, "sample-task", "T")
        self.assertEqual(result["sources"], ["state.md", "progress.md", "issues.md"])
        ...
        self.assertEqual(
            result["sources"],
            ["state.md", "progress.md", "issues.md", "docs/work-log-archive.md"],
        )
        self.assertEqual(result["max_used"], "T-0009")
        self.assertEqual(result["next_id"], "T-0010")

    def test_task_id_cited_only_in_issues_blocks_reuse(self) -> None:
        self.create()
        issues = self.task_path("issues.md")
        issues.write_text(
            issues.read_text(encoding="utf-8").replace(
                "## 未解決\n\nなし。\n",
                "## 未解決\n\n### I-0001 — 再検討\n\n- 発見: T-0042 の完了後に再検討する。\n",
            ),
            encoding="utf-8",
        )
        result = longneckctl.next_id(self.root, "sample-task", "T")
        self.assertEqual(result["max_used"], "T-0042")
        self.assertEqual(result["next_id"], "T-0043")

    def test_next_issue_id_scans_state_progress_and_issues(self) -> None:
        self.create()
        fresh = longneckctl.next_id(self.root, "sample-task", "I")
        self.assertEqual(fresh["sources"], ["state.md", "progress.md", "issues.md"])
        self.assertIsNone(fresh["max_used"])
        ...
        # An issue promoted into state.md keeps its I-ID visible there; that
        # citation alone must block the ID from being reused.
        state = self.task_path("state.md")
        state.write_text(
            state.read_text(encoding="utf-8").replace(
                "初期状態。", "初期状態。I-0007 を T-0001 として実行中。"
            ),
            encoding="utf-8",
        )
        promoted = longneckctl.next_id(self.root, "sample-task", "I")
        self.assertEqual(promoted["max_used"], "I-0007")
        self.assertEqual(promoted["next_id"], "I-0008")
    ...
    def test_description_sections_must_be_in_template_order(self) -> None:
        self.create()
        description = self.task_path("description.md")
        text = description.read_text(encoding="utf-8")
        swapped = text.replace(
        ...
        self.assertNotEqual(text, swapped)
        description.write_text(swapped, encoding="utf-8")
        result = longneckctl.validate_task(self.root, "sample-task")
        self.assertFalse(result.ok)
        self.assertTrue(
            any("description.md sections are out of order" in error for error in result.errors)
        )

    def test_nameless_decision_and_issue_headings_are_warnings(self) -> None:
        self.create()
        progress = self.task_path("progress.md")
        progress.write_text(
            progress.read_text(encoding="utf-8").replace(
        ...
        result = longneckctl.validate_task(self.root, "sample-task")
        self.assertTrue(result.ok, result.errors)
        self.assertIn("decision D-0001 has no name in its heading", result.warnings)
        self.assertIn("decision D-0002 has no name in its heading", result.warnings)
        self.assertIn("issue I-0001 has no name in its heading", result.warnings)
