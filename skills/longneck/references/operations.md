...
- 対象titleの文書を変更するスキル（`longneck` 親、`longneck-amend`、
  `longneck-repair`、`longneck-flush`、`longneck-sever`）は、対象の確定後、
  最初の書込みより前に `lock` を取得する。対象の文書は変更しないが調査中の
  情報源が不変であることに依存する直接起動された `longneck-persist` は、
  [persistence.md](persistence.md) に従い調査を始める前に取得する。どちらも
